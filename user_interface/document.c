...
/* this is a helper function that helps to determine a empty_line */ 
...
/* paragraphs, and separately the paragraphs of the document */
...
int add_paragraph_after(Document *doc, int paragraph_number) {
	...
	int para_num;
	para_num = doc->number_of_paragraphs;
	if (doc == NULL|| para_num == MAX_PARAGRAPHS || paragraph_number < 0
...
int add_line_after(Document *doc, int paragraph_number, int line_number,
	...
	/* The function will first move the lines which were previously located in */
	/* the place where we want to add the new line forward, then add the line */
...
int load_document(Document *doc, char data[][MAX_STR_SIZE + 1], int data_lines) {
	...
	for (i = doc->number_of_paragraphs - 1; i >= 0; i--) {
		doc->paragraphs[i + para_cnt].number_of_lines =
				doc->paragraphs[i].number_of_lines;
		for (j = 0; j < doc->paragraphs[i].number_of_lines; j++) {
			strcpy(doc->paragraphs[i + para_cnt].lines[j],
					doc->paragraphs[i].lines[j]);
		}
	}
	...
	for (i = 0; i < data_lines; i++) {
		if (is_empty_line(data[i])) {
			...
			strcpy(
		doc->paragraphs[para_idx].lines[doc->paragraphs[para_idx].number_of_lines],
					data[i]);
...
int highlight_text(Document *doc, const char *target) {
	...
	/* Put the highlighted target into the variable replacement */
...
/* This function adds paragraphs and rows from */
/* a file to the beginning of the document */
int load_file(Document *doc, const char *filename) {
	FILE *input;
	char line[MAX_LEN + 1];
	char data[100][MAX_STR_SIZE + 1];
	int data_lines = 0;
	...
	while (!feof(input)) {
		...
		strcpy(data[data_lines], line);
		data_lines++;
		fgets(line, MAX_LEN + 1, input);
	}
	fclose(input);
	return load_document(doc, data, data_lines);
}

/* this function prints the document to the specified file */ 
int save_document(Document *doc, const char *filename) {
	...
	for (i = 0; i < para_num; i++) {
		...
		/* This if condition helps to put a empty line between paragraphs */
		if (i < para_num - 1) {
			fputs("\n", input);
		}
	}
